...
class TestMeetups(Settings):
    ...
    def test_get_single_meetup(self):
        ...
        rv1 = self.client.get('/api/v1/meetups/upcoming/1')
    ...
    def test_get_all_meetups(self):
        ...
        self.assertEqual(res.status_code, 201)
        res1 = self.client.get('/api/v1/meetups/upcoming')
    ...
    def test_get_meetup_id_that_doesnt_exist(self):
        res = self.client.post('/api/v1/meetups/upcoming', data=json.dumps(self.meetup), content_type='application/json')
        self.assertEqual(res.status_code, 201)
        res1 = self.client.get('/api/v1/meetups/upcoming/4') 
        data = json.loads(res1.get_data().decode())
    ...
    def test_update_meetup(self):
        ...
        rv = self.client.post('/api/v1/meetups/upcoming', data=json.dumps(self.meetup), content_type='application/json')
        self.assertEqual(rv.status_code, 201)
        rv1 = self.client.put('/api/v1/meetups/upcoming/1', data=json.dumps(self.meetup), content_type='application/json')
        self.assertEqual(rv1.status_code, 201)
        results = self.client.get('/api/v1/meetups/upcoming/1')
